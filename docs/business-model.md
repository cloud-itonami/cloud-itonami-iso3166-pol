# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Poland

## Classification

- Repository: `cloud-itonami-iso3166-pol`
- ISO 3166: `POL` (Poland)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Polish
  public contract
...
- registration walkthrough for e-Zamówienia, Poland's central public-procurement platform, governed by the Public Procurement Law (Prawo zamówień publicznych) implementing EU procurement directives
- business/tax registration checklist: a KRS (Krajowy Rejestr Sądowy, National Court Register) entry plus a NIP (tax identification number); VAT (VAT-R) registration with the tax office where the taxable-turnover threshold is met
- local-content / preferential-procurement navigation: EU-wide open tendering above EU thresholds (no national-content quota as an EU member state), but Polish-language submission and a domestic/EU address for service are typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Polish-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Poland).
