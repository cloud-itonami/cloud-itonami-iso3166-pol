...
2. Register the client's intake: business type, target public function,
   prior filing history in Poland if any.
3. Run the advisor in read-only mode against e-Zamówienia, Poland's central public-procurement platform, governed by the Public Procurement Law (Prawo zamówień publicznych) implementing EU procurement directives.
...
- named referral relationship with Polish-licensed counsel or a
...
a working referral relationship with Polish-licensed counsel or a
registered agent for whatever licensed representation the law of
Poland requires for actual public-procurement filings.
